...
class Hessian2ServiceParser extends Hessian2Parser{
	...
	function parseFault(){
		...
		/** 异常框架修改20130822 start **/
		if (isset($map['detail']) && ($map["detail"] instanceof CscBizExceptionDTO)) {
			$map["detail"] 	= $map["detail"]->details;
		}else {
			$map['detail']	= $map;
		}
		/**	异常框架修改20130822 end **/
		$fault = new HessianFault(strval($map['message']), $map['code'], $map['detail']);
		throw $fault;
	}
